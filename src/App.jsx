...
import { 
  ...
  CalendarDays, BrainCircuit, Loader2, LogOut, Lock
} from 'lucide-react';
...
// ==========================================
const DEFAULT_API_KEY = 'AIzaSyBli7xNmfqYkj1pSPAHEscu-0rj6-a4bL0'; // Default Gemini API
const DEFAULT_COMMODITIES = ['Tembaga', 'Kuningan', 'Alumunium', 'Besi', 'Plastik', 'Kardus', 'Lainnya'];

const VEHICLES = [
  { id: 'sendiri', name: 'Mobil/Motor Sendiri (Tanpa Jasa)', basePrice: 0, baseKm: 0, pricePerKm: 0, capacity: 'Fleksibel' },
...
const DEFAULT_DB_STATE = {
   ...
   masterCommodities: DEFAULT_COMMODITIES,
...
export default function App() {
  ...
  if (loadingAuth) {
    return <div className="min-h-screen flex items-center justify-center bg-slate-900 text-white font-bold animate-pulse">Memuat Sistem Cloud...</div>;
  }
    ...
    <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4">
       <div className="bg-white p-8 rounded-3xl shadow-2xl w-full max-w-md border-t-8 border-t-amber-500">
          <div className="text-center mb-8">
             ...
             <p className="text-sm text-slate-500 font-medium">Sistem ERP Cloud UKM</p>
          </div>
          ...
          <form onSubmit={handleAuth} className="space-y-4">
             <div>
                <label className="block text-xs font-bold text-slate-600 uppercase mb-1">Email / Akun Bisnis</label>
                <div className="relative">
                   <UserRound className="absolute left-3 top-3 text-slate-400 w-5 h-5"/>
                   <input type="email" value={email} onChange={e=>setEmail(e.target.value)} className="w-full p-3 pl-10 border border-slate-300 rounded-xl focus:ring-2 focus:ring-amber-500 outline-none transition" placeholder="email@bisnis.com" required/>
                </div>
             </div>
             <div>
                ...
                <div className="relative">
                   ...
                   <input type="password" value={password} onChange={e=>setPassword(e.target.value)} className="w-full p-3 pl-10 border border-slate-300 rounded-xl focus:ring-2 focus:ring-amber-500 outline-none transition" placeholder="••••••••" required minLength="6"/>
                </div>
             </div>
             <button type="submit" disabled={isLoading} className="w-full bg-slate-900 hover:bg-slate-800 text-white font-bold py-3.5 rounded-xl shadow-lg transition mt-4 flex justify-center items-center gap-2">
...
function MainDashboard({ user }) {
  ...
  // FETCH DATA DARI FIREBASE SAAT PERTAMA LOGIN
  useEffect(() => {
    const fetchCloudData = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          // Proteksi mapping data (kalau ada fitur baru)
          ...
          // Jika belum pernah pakai sama sekali, buat database baru
          setDb(DEFAULT_DB_STATE);
          await setDoc(docRef, DEFAULT_DB_STATE);
        }
      } catch (err) {
        console.error("Gagal menarik data cloud", err);
        alert("Gagal memuat data Cloud. Pastikan koneksi internet stabil.");
      } finally {
        setIsDbLoaded(true);
      }
    };
  ...
  // AUTO-SAVE KE FIREBASE CLOUD (DEBOUNCED)
  useEffect(() => {
    ...
    const saveToCloud = setTimeout(async () => {
      try {
        ...
        console.error("Gagal auto-save ke cloud", err);
      }
    }, 1500); // Jeda 1.5 detik agar tidak membebani server Google saat ngetik cepat
  ...
  if (!isDbLoaded || !db) {
     return <div className="min-h-screen flex items-center justify-center bg-slate-50 text-slate-800 font-bold"><Loader2 className="w-8 h-8 animate-spin text-amber-500 mr-2"/> Menyiapkan Ruang Kerja Cloud...</div>;
  }

  return (
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row font-sans text-slate-800 print-wrapper">
      
      {/* AREA CETAK NOTA ROOT */}
          ...
          <>
            ...
            <div style={{ borderTop: '1px dashed #000', paddingTop: '5px', borderBottom: '1px dashed #000', paddingBottom: '5px', marginBottom: '10px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', color: '#555', fontSize: '10px' }}>
                 <span>Total Barang:</span><span>{formatRp(printNotaData.totalBarang)}</span>
              </div>
              {printNotaData.potongHutang > 0 && (
                <div style={{ display: 'flex', justifyContent: 'space-between', color: '#555', fontSize: '10px' }}>
                   <span>Potong Kasbon:</span><span>-{formatRp(printNotaData.potongHutang)}</span>
                </div>
              )}
              <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 'bold', marginTop: '3px', fontSize: '12px' }}>
                 <span>TOTAL DIBAYAR:</span><span>{formatRp(printNotaData.finalTotal)}</span>
              </div>
      ...
      {printNotaData && (
        <div className="fixed inset-0 bg-slate-900/80 z-[100] flex items-center justify-center p-4 hide-on-print backdrop-blur-sm">
          <div className="bg-white p-8 rounded-2xl shadow-2xl max-w-sm w-full text-center">
            ...
            <p className="text-slate-500 mb-8 font-medium">Data masuk ke gudang. Siapkan printer thermal Anda.</p>
            <button onClick={() => window.print()} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 mb-3 shadow-lg shadow-blue-500/30 transition text-lg">
              <Printer className="w-5 h-5"/> Cetak Nota
            </button>
            <button onClick={() => setPrintNotaData(null)} className="w-full text-slate-400 hover:text-slate-600 py-3 font-bold transition">
              Selesai & Tutup
            </button>
          </div>
        </div>
      ...
      {/* SIDEBAR NAVIGATION */}
      <aside className="w-full md:w-64 bg-slate-900 text-white flex flex-col shadow-2xl z-20 hide-on-print h-screen sticky top-0 overflow-y-auto">
        <div className="p-6">
        ...
        <nav className="flex-1 px-4 space-y-1.5 pb-4">
          <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2 mt-2 px-2">Transaksi</div>
          <NavBtn active={activeTab === 'dompet'} onClick={() => setActiveTab('dompet')} icon={<Wallet />} text="Dompetku (Kas)" />
          <NavBtn active={activeTab === 'pembelian'} onClick={() => setActiveTab('pembelian')} icon={<PackagePlus />} text="Pembelian (Masuk)" />
          <NavBtn active={activeTab === 'penjualan'} onClick={() => setActiveTab('penjualan')} icon={<ShoppingCart />} text="Penjualan (Keluar)" />
          
          <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2 mt-4 px-2">Manajemen</div>
          <NavBtn active={activeTab === 'gudang'} onClick={() => setActiveTab('gudang')} icon={<Database />} text="Gudang & Mitra" />
          <NavBtn active={activeTab === 'hutang'} onClick={() => setActiveTab('hutang')} icon={<CreditCard />} text="Buku Kasbon (Hutang)" />
          <NavBtn active={activeTab === 'operasional'} onClick={() => setActiveTab('operasional')} icon={<Settings />} text="Biaya Tetap" />
          <NavBtn active={activeTab === 'operasional_var'} onClick={() => setActiveTab('operasional_var')} icon={<Receipt />} text="Biaya Variabel" />
          
          <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2 mt-4 px-2">Analisis & Alat</div>
          <NavBtn active={activeTab === 'analisis'} onClick={() => setActiveTab('analisis')} icon={<LineChart />} text="Analisis Bisnis (AI)" />
          <NavBtn active={activeTab === 'laporan'} onClick={() => setActiveTab('laporan')} icon={<BarChart3 />} text="Laporan Keuangan" />
          <NavBtn active={activeTab === 'kalkulator'} onClick={() => setActiveTab('kalkulator')} icon={<Calculator />} text="Kalkulator Harga" />
          <NavBtn active={activeTab === 'profil'} onClick={() => setActiveTab('profil')} icon={<UserRound />} text="Profil Toko" />
        </nav>

        <div className="p-4 border-t border-slate-800 bg-slate-900/50">
          <div className="flex items-center gap-2 text-[11px] text-emerald-400 bg-slate-800/80 p-2 rounded-md border border-slate-700/50 mb-3">
          ...
          <div className="bg-slate-800/50 p-3 rounded-lg border border-slate-700">
             ...
             <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 bg-red-600/20 hover:bg-red-600 text-red-400 hover:text-white py-2 rounded text-xs transition font-bold border border-red-900/50 hover:border-red-600">
                <LogOut className="w-3.5 h-3.5" /> Logout Sistem
             </button>
          </div>
        </div>
      </aside>
        ...
        {`
          ...
          @media print {
            ...
            * { overflow: visible !important; float: none !important; }
            ...
            .print-only {
              display: block !important; position: absolute !important; left: 0 !important; top: 0 !important; width: 58mm !important; padding: 2mm !important; margin: 0 !important;
              font-family: 'Courier New', Courier, monospace !important; font-size: 11px !important; color: #000 !important; line-height: 1.2 !important; background-color: white !important;
            }
          }
        `}
      </style>
...
const NavBtn = ({ active, onClick, icon, text }) => (
  <button 
    onClick={onClick}
    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition duration-200 ${
      active 
...
const PageAnalisis = ({ db }) => {
  ...
  const monthlyData = useMemo(() => {
    ...
    (db.sales || []).forEach(s => {
       ...
       initMonth(m);
       data[m].income += s.pendapatan;
       data[m].hpp += s.hpp;
    });

    (db.variableCosts || []).forEach(v => {
       const m = v.tanggal.substring(0, 7);
       initMonth(m);
       data[m].opsVar += v.harga;
    });
  ...
  const insights = useMemo(() => {
     ...
     if (current.opsVar > prev.opsVar * 1.5 && current.opsVar > 100000) {
        alerts.push({ type: 'warning', text: `Hati-hati! Biaya operasional harian (variabel) membengkak ekstrim pada bulan ${current.month}. Lakukan efisiensi bensin/kuli.` });
     }
  ...
  const handleRequestAI = async () => {
     if (!apiKey) {
        alert("API Key Gemini belum diisi!\n\nSilakan buka menu 'Profil Toko' lalu masukkan API Key Anda di kolom yang tersedia.");
        return;
     }
     ...
     const promptData = `
        ...
        Berikut adalah data rekap keuangan bulanan saya:
        ${JSON.stringify(monthlyData, null, 2)}
        
        Total Uang Kasbon/Piutang di tangan pengepul saat ini: Rp ${totalKasbon}.
        Saldo Kas Saat ini: Rp ${db.wallet?.balance || 0}.

        Tolong bertindak sebagai Konsultan Bisnis Profesional. 
        Analisis data tersebut dan berikan saya:
        1. Analisis singkat performa bisnis saya (apakah sehat/berbahaya).
        2. Temukan kelemahan atau kebocoran dana dari data tersebut.
        3. Berikan 3 rekomendasi strategi konkrit dan praktis agar bisnis rongsok saya bisa lebih untung dan berkembang bulan depan.
        Jawab dengan bahasa Indonesia yang ramah, profesional, dan mudah dipahami.
     `;
     ...
     try {
        ...
        setAiResponse(`Gagal menghubungi AI. Pastikan koneksi internet aktif dan API Key valid.\nError: ${err.message}`);
     } finally {
  ...
  return (
     <div className="max-w-6xl mx-auto space-y-6">
        <div className="bg-slate-900 p-6 md:p-8 rounded-2xl shadow-xl text-white flex flex-col md:flex-row justify-between items-center gap-6">
           <div>
              <h2 className="text-2xl font-bold flex items-center gap-3"><LineChart className="text-amber-500 w-8 h-8" /> Analisis & Performa Bisnis</h2>
              <p className="text-slate-400 text-sm mt-1">Pantau grafik pertumbuhan dan minta saran strategi dari Konsultan AI.</p>
           </div>
           {!apiKey ? (
              <div className="bg-red-500/20 border border-red-500 text-red-200 px-4 py-2 rounded-lg text-xs font-bold text-center">
                 API Key Gemini Belum Diatur.<br/><span className="font-normal opacity-80">Atur di menu Profil Toko.</span>
              </div>
           ) : (
              <div className="bg-emerald-500/20 border border-emerald-500 text-emerald-200 px-4 py-2 rounded-lg text-xs font-bold text-center">
                 AI Terhubung <CheckCircle2 className="w-3 h-3 inline"/>
              </div>
           )}
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 border-t-4 border-t-purple-500">
           <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
              <div>
                 <h3 className="font-bold text-slate-800 text-lg flex items-center gap-2"><BrainCircuit className="w-6 h-6 text-purple-500"/> Konsultan Bisnis AI (Gemini)</h3>
                 <p className="text-sm text-slate-500 mt-1">Sistem akan menganalisis data keuangan Anda dan memberikan saran strategi usaha.</p>
              </div>
              <button 
                 onClick={handleRequestAI} 
                 disabled={isLoadingAi || monthlyData.length === 0}
                 className={`px-6 py-3 rounded-xl font-bold shadow-md transition flex items-center gap-2 ${isLoadingAi || monthlyData.length === 0 ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-purple-600 hover:bg-purple-500 text-white'}`}
              >
                 {isLoadingAi ? <><Loader2 className="w-5 h-5 animate-spin"/> Menganalisis...</> : <><BrainCircuit className="w-5 h-5"/> Minta Analisis & Strategi AI</>}
              </button>
           </div>

           {aiResponse && (
              <div className="bg-purple-50 border border-purple-100 p-6 rounded-xl text-slate-700 text-sm leading-relaxed whitespace-pre-wrap">
                 {aiResponse}
              </div>
           )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
           <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 lg:col-span-2">
              <h3 className="font-bold text-slate-800 mb-6 flex items-center gap-2"><Activity className="w-5 h-5 text-blue-500"/> Grafik Pendapatan vs Pengeluaran per Bulan</h3>
              {monthlyData.length === 0 ? (
                 <div className="h-64 flex items-center justify-center text-slate-400 border-2 border-dashed rounded-xl">Belum ada data transaksi yang bisa dianalisis.</div>
              ) : (
                       ...
                       return (
                          <div key={i} className="flex flex-col items-center gap-2 group flex-shrink-0 min-w-[80px]">
                             <div className="flex items-end gap-1 h-48 w-full relative">
                                <div className="w-1/2 bg-blue-500 rounded-t-sm hover:bg-blue-400 transition-all relative" style={{ height: `${incomeHeight}%` }}>
                                   <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[9px] py-0.5 px-1.5 rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-10">{formatRp(d.income)}</div>
                                </div>
                                <div className="w-1/2 bg-red-400 rounded-t-sm hover:bg-red-300 transition-all relative" style={{ height: `${expenseHeight}%` }}>
                                   <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 bg-slate-800 text-white text-[9px] py-0.5 px-1.5 rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-10">{formatRp(d.totalExpense)}</div>
                                </div>
                             </div>
              ...
              <div className="flex gap-6 mt-4 justify-center text-xs font-bold text-slate-500">
           ...
           <div className="overflow-x-auto">
              <table className="w-full text-left text-sm border-collapse">
                 <thead className="bg-slate-50 border-b border-slate-200">
                    <tr><th className="p-3">Bulan</th><th className="p-3 text-right">Pendapatan Kotor</th><th className="p-3 text-right">HPP (Modal)</th><th className="p-3 text-right">Ops Variabel</th><th className="p-3 text-right">Ops Tetap</th><th className="p-3 text-right">Laba Bersih</th></tr>
                 </thead>
                 <tbody className="divide-y divide-slate-100">
                    {monthlyData.slice().reverse().map((d, i) => (
                       <tr key={i} className="hover:bg-slate-50">
                          <td className="p-3 font-bold text-slate-700">{d.month}</td>
                          <td className="p-3 text-right text-blue-600 font-medium">{formatRp(d.income)}</td>
                          <td className="p-3 text-right text-red-400">-{formatRp(d.hpp)}</td>
                          <td className="p-3 text-right text-red-400">-{formatRp(d.opsVar)}</td>
                          <td className="p-3 text-right text-red-400">-{formatRp(d.opsTetap)}</td>
                          <td className={`p-3 text-right font-black ${d.netProfit >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{formatRp(d.netProfit)}</td>
                       </tr>
                    ))}
                 </tbody>
              </table>
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="bg-gradient-to-br from-emerald-600 to-emerald-800 p-8 md:p-10 rounded-3xl shadow-xl text-white relative overflow-hidden flex flex-col items-center justify-center">
         ...
         <div className="relative z-10 text-center">
            ...
            <div className="text-5xl md:text-7xl font-black drop-shadow-md mb-2">{formatRp(currentBalance)}</div>
...
// PAGE PEMBELIAN
// ==========================================
const PagePembelian = ({ db, setDb, setPrintNotaData }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (potongHutangVal > 0 && activeDebtor) {
       ...
       if (debtorIndex >= 0) {
          updatedDebtors[debtorIndex].history.push({ id: Date.now(), tanggal: tanggal, type: 'LUNAS', amount: potongHutangVal, note: `Potong otomatis dari TRX Barang` });
       }
    }
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200">
             <div className="flex justify-between items-center border-b border-slate-100 pb-4 mb-6">
                ...
                <input type="date" value={tanggal} onChange={e=>setTanggal(e.target.value)} className="border border-slate-200 rounded-md px-3 py-1.5 bg-slate-50 outline-none text-sm font-bold text-slate-600" />
             </div>
             <div className="space-y-4">
               {items.map((item, index) => (
                 <div key={item.id} className="flex flex-col md:flex-row gap-4 items-end bg-slate-50 p-4 rounded-xl border border-slate-200 relative group">
                   ...
                   <button type="button" onClick={() => removeItemRow(item.id)} className={`pb-2 transition flex-shrink-0 ${items.length > 1 ? 'text-red-400 hover:text-red-600 hover:scale-110' : 'text-slate-200 cursor-not-allowed'}`} disabled={items.length <= 1}><XCircle className="w-8 h-8" /></button>
                 </div>
               ))}
             </div>
             ...
             {sisaHutangSeller > 0 && (
                <div className="mt-8 bg-red-50 border border-red-200 p-6 rounded-xl flex flex-col md:flex-row items-center gap-6">
                   <div className="flex-1"><h4 className="font-bold text-red-800 flex items-center gap-2 mb-1"><AlertTriangle className="w-5 h-5" /> Opsi Potong Kasbon</h4><p className="text-sm text-red-600 font-medium">Sisa Pinjaman Kasbon: <strong>{formatRp(sisaHutangSeller)}</strong>.</p></div>
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 text-white p-6 md:p-8 rounded-2xl shadow-xl sticky top-6">
            ...
            <div className="border-t border-slate-700 pt-4 mb-8">
               ...
               <div className="text-4xl font-black text-amber-400 break-words">{formatRp(finalTotalBayar)}</div>
            </div>
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200">
        ...
        <form onSubmit={handleJual} className="space-y-6">
          ...
          <div className="bg-slate-900 p-6 rounded-2xl text-white flex flex-col md:flex-row justify-between items-center mt-6 shadow-xl"><div className="text-center md:text-left mb-4 md:mb-0"><div className="text-sm text-slate-400 font-medium">TOTAL MASUK KAS:</div><div className="text-3xl font-black text-amber-400">{formatRp(Number(beratJual) * Number(hargaJual))}</div></div><button type="submit" className="w-full md:w-auto bg-blue-600 hover:bg-blue-500 px-10 py-4 rounded-xl font-bold shadow-lg transition"><ArrowRightLeft className="inline w-5 h-5 mr-2" /> Proses Penjualan</button></div>
        </form>
      </div>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="bg-slate-900 p-6 md:p-8 rounded-2xl shadow-xl text-white flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
         ...
         <div className="relative z-10 bg-slate-800 p-4 rounded-xl border border-slate-700 text-center"><div className="text-xs font-bold text-slate-400 mb-1 uppercase tracking-widest">TOTAL UANG DI LUAR</div><div className="text-3xl font-black text-red-400">{formatRp(globalTotalPiutang)}</div></div>
      </div>
      <div className="flex flex-wrap gap-2">
        <button onClick={() => setTab('list')} className={`px-6 py-2.5 rounded-lg font-bold text-sm ${tab==='list' ? 'bg-blue-600 text-white' : 'bg-white text-slate-600'}`}>Daftar Pengutang</button>
        <button onClick={() => setTab('pinjam')} className={`px-6 py-2.5 rounded-lg font-bold text-sm ${tab==='pinjam' ? 'bg-amber-500 text-slate-900' : 'bg-white text-slate-600'}`}>+ Beri Pinjaman</button>
        <button onClick={() => setTab('lunas')} className={`px-6 py-2.5 rounded-lg font-bold text-sm ${tab==='lunas' ? 'bg-emerald-500 text-white' : 'bg-white text-slate-600'}`}>✓ Terima Cicilan Manual</button>
      </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
           {selectedDebtor ? (
             <div className="p-6">
                ...
                <div className="flex justify-between items-end border-b pb-4 mb-6"><div><h3 className="text-2xl font-black text-slate-800">{selectedDebtor.nama}</h3></div><div className="text-right"><div className="text-xs font-bold text-slate-500 uppercase">Sisa Hutang</div><div className="text-2xl font-black text-red-600">{formatRp(selectedDebtor.sisa)}</div></div></div>
...
// ==========================================
const PageOperasionalTetap = ({ db, setDb }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200 border-l-4 border-l-blue-500">
         <h2 className="text-2xl font-bold mb-2 flex items-center gap-3 text-blue-600"><WalletCards className="w-6 h-6"/> Bayar Tagihan Bulan Ini</h2>
      ...
      <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200">
         ...
         <div className="space-y-3">
            {db.fixedCosts.map(item => {
               ...
               return (
                 <div key={item.id} className="flex justify-between items-center p-4 bg-white border border-slate-200 rounded-xl shadow-sm hover:border-slate-300 transition">
...
// ==========================================
const PageOperasionalVariabel = ({ db, setDb }) => {
  const [tanggal, setTanggal] = useState(new Date().toISOString().split('T')[0]);
  const [nama, setNama] = useState('');
  const [harga, setHarga] = useState('');
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200">
       <h2 className="text-2xl font-bold mb-2 flex items-center gap-3"><Receipt className="text-blue-500"/> Operasional Variabel (Harian)</h2>
       <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8 bg-blue-50/50 p-6 rounded-xl border border-blue-100">
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-10">
      ...
      <div className="bg-red-50 p-6 rounded-2xl border-2 border-red-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4">
         <div>
            <h3 className="text-red-800 font-bold text-lg flex items-center gap-2"><CreditCard className="w-5 h-5"/> Piutang Lapangan (Modal Dipinjam)</h3>
            <p className="text-sm text-red-600 mt-1">Uang modal tertahan di tangan mitra dalam bentuk kasbon.</p>
         </div>
         <div className="text-3xl font-black text-red-600 bg-white px-6 py-3 rounded-xl shadow-inner border border-red-100">{formatRp(totalPiutang)}</div>
      </div>

      <div className="bg-slate-900 text-white p-6 md:p-8 rounded-2xl shadow-xl">
         <h2 className="text-2xl font-bold mb-2 text-amber-500">Pusat Kendali Harga Aset Gudang</h2>
         <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 mt-4">
            {commoditiesList.map(c => (
               <div key={c} className="bg-slate-800 p-3 rounded-xl border border-slate-700 shadow-inner">
                  <label className="block text-xs text-slate-300 font-bold mb-2 uppercase tracking-wide">{c}</label>
                  <input type="number" value={db.globalPrices[c] || ''} onChange={(e) => handlePriceChange(c, e.target.value)} className="w-full bg-slate-900 text-amber-400 text-lg font-black p-2.5 rounded-lg border border-slate-600 outline-none focus:border-amber-500 transition text-center" placeholder="Rp..."/>
               </div>
            ))}
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
         <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border-2 border-emerald-500 relative">
            <div className="absolute top-0 right-0 bg-emerald-500 text-white text-xs font-bold px-4 py-1.5 rounded-bl-xl shadow-sm">KAS NYATA</div>
            <h3 className="font-bold text-2xl mb-8 flex items-center gap-3 text-slate-800"><WalletCards className="w-6 h-6 text-emerald-500"/> Laporan Uang Cair</h3>
            <div className="space-y-4 mb-8">
               <div className="flex justify-between items-center text-slate-600"><span className="text-sm">Uang Masuk (Penjualan)</span><span className="font-black text-lg text-blue-600">{formatRp(totalPendapatanCair)}</span></div>
               <div className="flex justify-between items-center text-slate-600"><span className="text-sm">Modal Terjual (HPP)</span><span className="font-bold text-slate-700">- {formatRp(totalModalHppCair)}</span></div>
               <div className="flex justify-between items-center pt-3 border-t border-slate-200">
                  <span className="text-sm font-bold text-slate-800">Gross Profit</span>
                  <span className={`font-black text-xl ${grossProfitCair >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{formatRp(grossProfitCair)}</span>
               </div>
               <div className="flex justify-between items-center text-red-500 pt-2 bg-red-50 p-3 rounded-lg border border-red-100">
                  <span className="text-sm font-semibold">Beban Operasional</span><span className="font-bold">- {formatRp(totalBebanOperasional)}</span>
               </div>
            </div>
            <div className={`p-6 rounded-xl border-2 text-center shadow-inner ${isNetCairProfit ? 'bg-emerald-50 border-emerald-200' : 'bg-red-50 border-red-200'}`}>
               <div className="text-sm font-bold text-slate-700 tracking-widest mb-2">KEUNTUNGAN BERSIH CAIR</div>
               <div className={`text-4xl md:text-5xl font-black ${isNetCairProfit ? 'text-emerald-600' : 'text-red-600'}`}>{formatRp(netProfitCair)}</div>
            </div>
         </div>

         <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border-2 border-amber-400 relative flex flex-col">
            <div className="absolute top-0 right-0 bg-amber-400 text-slate-900 text-xs font-bold px-4 py-1.5 rounded-bl-xl shadow-sm">ASET BARANG</div>
            <h3 className="font-bold text-2xl mb-8 flex items-center gap-3 text-slate-800"><PackagePlus className="w-6 h-6 text-amber-500"/> Laporan Aset Gudang</h3>
            <div className="space-y-4 mb-8">
               <div className="flex justify-between items-center text-slate-600"><span className="text-sm">Modal Tertahan di Gudang</span><span className="font-bold text-slate-800">{formatRp(totalModalMengendap)}</span></div>
               <div className="flex justify-between items-center text-slate-600"><span className="text-sm">Estimasi Jual Sekarang</span><span className="font-black text-lg text-blue-600">{formatRp(totalNilaiMengendap)}</span></div>
            </div>
            <div className={`p-6 rounded-xl border text-center shadow-inner ${potensiUntungMengendap >= 0 ? 'bg-blue-50 border-blue-200' : 'bg-red-50 border-red-200'}`}>
               <div className="text-sm font-bold text-slate-700 tracking-widest mb-2">POTENSI UNTUNG MENGENDAP</div>
               <div className={`text-3xl md:text-4xl font-black ${potensiUntungMengendap >= 0 ? 'text-blue-600' : 'text-red-600'}`}>{potensiUntungMengendap >= 0 ? '+' : ''}{formatRp(potensiUntungMengendap)}</div>
            </div>
         </div>
      </div>
    </div>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 mt-4 pb-12">
      <div className="bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-slate-200">
        <h2 className="text-3xl font-black flex items-center gap-3 mb-2 text-slate-800"><UserRound className="w-8 h-8 text-blue-500"/> Profil & Pengaturan</h2>
        <p className="text-slate-500 mb-8 font-medium">Data ini digunakan untuk identitas Struk/Nota Kasir dan menghubungkan aplikasi dengan kecerdasan buatan.</p>
      ...
      <div className="bg-red-50 p-6 md:p-8 rounded-2xl border-2 border-red-200 shadow-sm mt-12">
        <h3 className="text-xl font-black text-red-700 flex items-center gap-2 mb-2"><AlertTriangle className="w-6 h-6" /> ZONA BERBAHAYA (Reset Data)</h3>
        ...
        <button onClick={handleResetData} type="button" className="bg-red-600 hover:bg-red-700 text-white font-bold py-3 px-6 rounded-lg transition shadow-md flex items-center gap-2"><Trash2 className="w-5 h-5" /> Bersihkan Seluruh Data (Factory Reset)</button>
      </div>
    </div>
  );
};